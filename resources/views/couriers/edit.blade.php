                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                            <div class="card-body" style="width: 700px">
                                ...
                                @endif
                                <form role="form" id="courierForm" action="{{ route('couriers.update', $courier->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <label>Name</label>
                                    <div class="mb-3">
                                        <input type="text" name="name" class="form-control" id="name" placeholder="Input Courier's Name"
                                            aria-label="Name" aria-describedby="name-addon" required value="{{ old('name', $courier->name) }}">
                                    </div>
                                    <label>Email</label>
                                    <div class="mb-3">
                                        <input type="email" name="email" class="form-control" id="email" placeholder="Input Courier's Email"
                                            aria-label="Email" aria-describedby="email-addon" required value="{{ old('email', $courier->email) }}">
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        <input type="text" name="phone_number" class="form-control" id="phone"
                                            ...
                                            aria-describedby="phoneNumber-addon" value="{{ old('phone_number', $courier->phone_number) }}">
                                    </div>
                                    <label>Address</label>
                                    <div class="mb-3">
                                        <textarea name="address" class="form-control" id="address"
                                            placeholder="Input Courier's Address" aria-label="Address"
                                            aria-describedby="address-addon">{{ old('address', $courier->address) }}</textarea>
                                    </div>
                                    <div class="text-end">
